...
func (s *postgresStore) ListIssues(ctx context.Context, filter ListIssuesFilter, limit int) ([]IssueSummary, error) {
	...
	if filter.Search != "" {
		q += fmt.Sprintf(" AND (i.title ILIKE '%%'||$%d||'%%' OR i.culprit ILIKE '%%'||$%d||'%%')", n, n)
...
func (s *postgresStore) GetEventVolumeByDay(ctx context.Context, days int) ([]DayCount, error) {
	since := unixNow() - int64(days)*86400
	rows, err := s.db.QueryContext(ctx,
		`SELECT DATE(to_timestamp(received_at)) AS day, COUNT(*)
...
func (s *postgresStore) ListReleaseStats(ctx context.Context) ([]ReleaseStat, error) {
	rows, err := s.db.QueryContext(ctx,
